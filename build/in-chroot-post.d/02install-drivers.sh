...
if [[ -z "$TINYBOX_CORE" ]]; then
  if [[ "$TINYBOX_COLOR" == "red" ]]; then
    ...
    echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/rocm.gpg] https://repo.radeon.com/amdgpu/6.4.1/ubuntu noble main" | sudo tee /etc/apt/sources.list.d/rocm.list
    echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/rocm.gpg] https://repo.radeon.com/rocm/apt/6.4.1 noble main" | sudo tee --append /etc/apt/sources.list.d/rocm.list
    ...
    curl -o driver.deb -L "https://github.com/wozeparrot/open-gpu-kernel-modules/releases/download/570.148.08-p2p/nvidia-kernel-source-570-open-1ubuntu1_amd64.deb"
    ...
    apt update -y
    apt install nvidia-persistenced=570.148.08-1ubuntu1 -y
    apt install nvidia-compute-utils-570 -y
